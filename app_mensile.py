...
with st.sidebar:
    ...
    with c1:
        _lang_it = st.sidebar.button("🇮🇹 IT", key="it-hidden", use_container_width=True)
    with c2:
        _lang_en = st.sidebar.button("🇬🇧 EN", key="uk-hidden", use_container_width=True)
    
    if _lang_it: st.session_state.lang = "it"; st.rerun()
    if _lang_en: st.session_state.lang = "en"; st.rerun()

_LANG = st.session_state.get("lang", "it")
...
with st.sidebar:
    ...
    _mc3, _mc4 = st.sidebar.columns(2)
    with _mc3:
        if st.sidebar.button(
            "⚡ CHP 2012",
            use_container_width=True,
    ...
    with st.expander(
        ...
        if "_error" in _norm_local:
            st.sidebar.error(f"❌ {_norm_local['_error']}")
        else:
            st.sidebar.caption(
            ...
            st.sidebar.markdown(f"**🟢 {len(_norme)} norme cablate nel codice:**")
            for n in _norme:
            ...
            if st.sidebar.button(
                ...
                if "_error" in _norm_remote:
                    st.sidebar.error(f"❌ {_norm_remote['_error']}")
                else:
                    ...
                    if _pendenti:
                        st.sidebar.markdown("**🕐 Note normative pendenti:**")
                        for p in _pendenti:
                            st.sidebar.markdown(
                                f"- **{p.get('norma', '?')}** "
...
with st.sidebar:
    ...
    # ============================================================
    # IMPIANTO & SCALA - Sidebar
    # ============================================================
    with st.sidebar.expander("🏭 " + _t("Impianto & Scala"), expanded=True):
        if IS_CHP:
            plant_kwe = st.sidebar.number_input(
                "🔌 " + _t("Potenza autorizzata [kWe lordi]"),
                min_value=10.0, max_value=FER2_KWE_CAP if IS_FER2 else 5000.0,
                value=float(st.session_state.get("plant_kwe_saved", 300.0)),
                step=10.0,
                key="sb_plant_kwe",
            )
            st.session_state.plant_kwe_saved = plant_kwe
            
            aux_el_pct = st.sidebar.slider(
                "⚙️ " + _t("Autoconsumo elettrico [%]"),
                0.0, 20.0, AUX_EL_DEFAULT * 100.0, 0.5, key="sb_aux_el"
            ) / 100.0
            plant_kwe_net = plant_kwe * (1.0 - aux_el_pct)
            plant_net_smch = plant_kwe / (eta_el * 9.97)
            
            st.sidebar.metric(_t("Netto in rete"), f"{fmt_it(plant_kwe_net, 0)} kWe")
        else:
            _unit_opts = [_t("Sm³/h NETTI"), _t("Sm³/h LORDI")]
            _unit_sel = st.sidebar.radio(_t("Inserisci taglia come"), _unit_opts, index=0, horizontal=True, key="sb_unit_mode")
            ...
            if _unit_sel == _unit_opts[0]: # NETTI
                ...
                if _sync:
                    ...
                    st.sidebar.caption(f"💡 {_t('Lordo calcolato')}: **{fmt_it(plant_gross_smch, 1)}**")
                else:
                ...
                if _sync:
                    ...
                    st.sidebar.caption(f"💡 {_t('Netto calcolato')}: **{fmt_it(plant_net_smch, 1)}**")
                else:
                    plant_net_smch = st.sidebar.number_input("📤 " + _t("Netto [Sm³/h]"), 10.0, plant_gross_smch, plant_gross_smch / _aux_base, 5.0, key="sb_net_smch_in")

        st.session_state["plant_net_smch_saved"]   = plant_net_smch
        st.session_state["plant_gross_smch_saved"] = plant_gross_smch
        
        # Riepilogo mini
        st.sidebar.markdown(f"**{fmt_it(plant_net_smch, 0)} Sm³/h netti**")

    # Per coerenza in mode biometano: eta_el/eta_th non usati in BP
    if not IS_CHP:
        eta_el = ETA_EL_DEFAULT; eta_th = ETA_TH_DEFAULT; aux_el_pct = 0.0
    ...
    bp_plant_type = BP_PLANT_TYPE_DEFAULT
    bp_dest_use = list(BP_DEST_USE.keys())[0]
    ghg_threshold = 0.80
    ...
    # ============================================================
    with st.sidebar.expander("🎯 " + _t("Config. Solver & Ottimizza"), expanded=False):
        st.sidebar.markdown(f"##### 🎯 {_t('Modalità di calcolo')}")
        ...
        optimize_clicked = st.sidebar.button(
            "🚀 " + _t("OTTIMIZZA"),
            help=f"Minimizza la massa totale soddisfacendo production e saving ({_n_combinations} combinazioni).",
            use_container_width=True,
            type="primary",
            key="sb_btn_optimize"
        )
        ...
        if optimize_clicked:
            best = find_optimal_pair(aux=aux_factor, plant_net=plant_net_smch, ep=ep_total, target_e_max=target_e_max, feed_list=active_feeds)
            if best is None:
                st.sidebar.error("❌ Nessuna soluzione trovata.")
            else:
                pair, total_h, masses_h = best
                active_masses = [n for n, v in masses_h.items() if v > 1e-9]
                st.session_state["_pending_optimization"] = {
                    "pair": list(pair), "unused": [n for n in active_feeds if n not in pair],
                    "total_year": total_h * sum(MONTH_HOURS), "is_mono": len(active_masses) == 1,
                    "mono": active_masses[0] if len(active_masses) == 1 else None
                }
                st.rerun()

        st.sidebar.divider()
        st.sidebar.caption(_t("Scegli le biomasse da gestire manualmente. Le altre verranno calcolate dal solver."))
        
        default_fixed = active_feeds[:min(2, N_active)]
        fixed_feeds = st.sidebar.multiselect(_t("Biomasse fisse"), active_feeds, default=st.session_state.get("fixed_multiselect_sb", default_fixed), key="fixed_multiselect_sb")
        unknown_feeds = [n for n in active_feeds if n not in fixed_feeds]
        
        if len(unknown_feeds) > 2:
            st.sidebar.warning("⚠️ Max 2 incognite.")
        
        if len(unknown_feeds) == 2:
            st.sidebar.info(f"**Dual-constraint**: {unknown_feeds[0]}, {unknown_feeds[1]}")
        elif len(unknown_feeds) == 1:
            st.sidebar.info(f"**Mono-constraint**: {unknown_feeds[0]}")
        else:
            st.sidebar.info("**Simulazione**: tutti i valori sono manuali.")

    # Handling pending optimization (from sidebar click)
    ...
    if _pending_opt is not None:
        ...
        if _pending_opt.get("is_mono"):
            ...
            st.session_state["fixed_multiselect_sb"] = others
            new_state_key = f"mens_in_1unk_{_opt_active_hash}_{'-'.join(others)}"
            st.session_state[new_state_key] = pd.DataFrame([{"Mese": mm, "Ore": hh, **{f: 0.0 for f in others}} for mm, hh in zip(MONTHS, MONTH_HOURS)])
        else:
            unused = _pending_opt["unused"]
            st.session_state["fixed_multiselect_sb"] = unused
            new_state_key = f"mens_in_2unk_{_opt_active_hash}_{'-'.join(unused)}"
            st.session_state[new_state_key] = pd.DataFrame([{"Mese": mm, "Ore": hh, **{f: 0.0 for f in unused}} for mm, hh in zip(MONTHS, MONTH_HOURS)])
        st.session_state["_optimize_info"] = _pending_opt

    # ============================================================
    # CONFIGURAZIONE TECNICA & GHG (ep, aux) - Sidebar
    # ============================================================
    with st.sidebar.expander("⚙️ " + _t("Config. Tecnica & GHG"), expanded=False):
        digestate_opt = st.sidebar.selectbox(_t("Stoccaggio digestato"), list(EP_DIGESTATE.keys()), index=1, key="sb_digestate")
        if IS_CHP:
            upgrading_opt = None; offgas_opt = None; injection_opt = None
            ep_upgrading = 0.0; ep_offgas = 0.0
        else:
            upgrading_opt = st.sidebar.selectbox(_t("Tecnologia upgrading"), list(EP_UPGRADING.keys()), index=1, key="sb_upgrading")
            offgas_opt    = st.sidebar.selectbox(_t("Combustione off-gas"), list(EP_OFFGAS.keys()), index=0, key="sb_offgas")
            ep_upgrading  = EP_UPGRADING[upgrading_opt]
            ep_offgas     = EP_OFFGAS[offgas_opt]
            injection_opt = st.sidebar.selectbox(_t("Iniezione in rete"), list(INJECTION_PRESSURE.keys()), index=1, key="sb_injection")

        heat_opt = st.sidebar.selectbox(_t("Fonte calore"), list(EP_HEAT.keys()), index=0, key="sb_heat")
        elec_opt = st.sidebar.selectbox(_t("Elettricità ausiliari"), list(EP_ELEC.keys()), index=1, key="sb_elec")
        ...
        st.sidebar.markdown(f"**ep totale: {fmt_it(ep_total, 1, signed=True)} gCO₂/MJ**")
        
        # aux_factor
        st.sidebar.markdown("---")
        margin_pct = st.sidebar.slider(_t("Margine/Downtime [%]"), 0.0, 10.0, 3.0, 0.5, key="sb_margin")
        if IS_CHP:
            aux_auto = 1.0 / max(1.0 - margin_pct / 100.0, 0.80)
            aux_auto_data = None; cogen_frac = 0.0; recover_chp_heat = False
        else:
            cogen_frac = 0.6; recover_chp_heat = True
            if elec_opt == ELEC_IS_INTERNAL:
                c1, c2 = st.sidebar.columns(2)
                cogen_frac = c1.number_input("% Cogen", 0.0, 100.0, 60.0, key="sb_cogen_f") / 100.0
                recover_chp_heat = c2.checkbox("Recupero Q", True, key="sb_rec_q")
            
            aux_auto_data = compute_aux_factor(upgrading_opt, heat_opt, elec_opt, injection_opt, margin_pct/100.0, cogen_frac, recover_chp_heat)
            aux_auto = aux_auto_data["aux_factor"]
            
        manual_aux_on = st.sidebar.checkbox(_t("Override aux manuale"), False, key="sb_aux_manual_on")
        if manual_aux_on:
            aux_factor = st.sidebar.number_input("aux manuale", 1.0, 2.0, round(aux_auto, 3), 0.005, key="sb_aux_manual_val")
        else:
            aux_factor = aux_auto
        ...
        st.sidebar.divider()
        st.sidebar.markdown(f"**ep: {fmt_it(ep_total, 1, signed=True)} gCO₂/MJ**")
        _ep_data = {"Digestato": ep_digestate, "Upgrading": ep_upgrading, "Off-gas": ep_offgas, "Calore": ep_heat, "Elettricità": ep_elec}
        _fig_ep = px.pie(values=list(_ep_data.values()), names=list(_ep_data.keys()), hole=0.4, height=200)
        _fig_ep.update_layout(margin=dict(l=0, r=0, t=0, b=0), showlegend=False)
        st.sidebar.plotly_chart(_fig_ep, use_container_width=True)
        
        if not IS_CHP and "aux_factor_detail" in st.session_state:
            _det = st.session_state.aux_factor_detail
            st.sidebar.markdown(f"**aux: {fmt_it(aux_factor, 3)}**")
    ...
    # ============================================================
    with st.sidebar.expander("🧪 " + _t("Override & Fattori Emissivi"), expanded=False):
        # BMT Override
        if "bmt_overrides" not in st.session_state:
            st.session_state["bmt_overrides"] = {}
        
        st.sidebar.markdown(f"#### 🧪 {_t('Resa BMT certificata')}")
        ...
        for _bmt_name in active_feeds:
            ...
            _bmt_active = st.sidebar.checkbox(
                f"🧪 {_bmt_name} (std {fmt_it(_std_y, 0)})",
                key=_key_active,
                value=st.session_state.get(_key_active, False),
            )
            if _bmt_active:
                _bmt_value = st.sidebar.number_input(f"Resa [{BMT_YIELD_UNIT}]", 0.0, 2000.0, _std_y, 1.0, key=f"bmt_val_v2_{_bmt_name}")
                _bmt_lab   = st.sidebar.text_input(_t("Lab"), key=f"bmt_lab_v2_{_bmt_name}")
                _bmt_cert  = st.sidebar.file_uploader(_t("Certificato"), type=["pdf","jpg","png","xlsx","csv"], key=f"bmt_file_v2_{_bmt_name}")
                
                _bmt_valid, _bmt_errs, _bmt_warns = validate_bmt_override(_bmt_value, _std_y, _bmt_cert is not None, _bmt_lab, "2026-01-01", "REF", _bmt_cert.name if _bmt_cert else "")
                if _bmt_valid:
                    st.session_state["bmt_overrides"][_bmt_name] = {
                        "active": True,
                        "certificate": BMTCertificate(_bmt_name, float(_bmt_value), _bmt_lab, "2026-01-01", "REF", _bmt_cert.name, _bmt_cert.size)
                    }
                    st.sidebar.success("✅ Active")
                else:
                    st.session_state["bmt_overrides"].pop(_bmt_name, None)
                    if _bmt_cert: st.sidebar.error("❌ Invalid")

        st.sidebar.markdown("---")
        st.sidebar.markdown(f"#### 🧬 {_t('Emissioni reali')}")
        ...
        for _ef_name in active_feeds:
            ...
            _key_active = f"ef_active_v2_{_ef_name}"
            _ef_active = st.sidebar.checkbox(f"🧬 {_ef_name}", key=_key_active)
            if _ef_active:
                _ef_eec  = st.sidebar.number_input("eec", -100.0, 100.0, float(_d_std["eec"]), 0.1, key=f"ef_eec_v2_{_ef_name}")
                _ef_etd  = st.sidebar.number_input("etd", 0.0, 50.0, float(_d_std["etd"]), 0.1, key=f"ef_etd_v2_{_ef_name}")
                _ef_file = st.sidebar.file_uploader(_t("Relazione"), type=["pdf","docx","xlsx"], key=f"ef_file_v2_{_ef_name}")
                if _ef_file:
                    st.session_state["emission_factor_overrides"][_ef_name] = {
                        "active": True,
                        "report": EmissionFactorReport(_ef_name, _ef_eec, 0.0, _ef_etd, 0.0, 0.0, "Relazione", "Tecnico", "Studio", "2026-01-01", "Impianto", "REF", "", _ef_file.name, _ef_file.size, EMISSION_UNIT)
                    }
                    st.sidebar.success("✅ Active")

        # --- Audit Table (Moved from Right) ---
        st.sidebar.divider()
        st.sidebar.caption(_t("Riepilogo Overrides"))
        _audit_data = []
        for _bn in active_feeds:
            _audit_data.append({
                "Biomassa": _bn,
                "Resa": "🧪 BMT" if _bn in st.session_state.get("bmt_overrides", {}) else "📚 STD",
                "GHG": "🧬 REAL" if _bn in st.session_state.get("emission_factor_overrides", {}) else "📚 STD"
            })
        st.sidebar.dataframe(pd.DataFrame(_audit_data), hide_index=True)

        # --- Detailed Audit Tables (Standard vs Used) ---
        if _yield_audit_rows:
            st.sidebar.divider()
            st.sidebar.markdown(f"##### 🧪 {_t('Dettaglio Audit Rese')}")
            st.sidebar.dataframe(pd.DataFrame(_yield_audit_rows), hide_index=True)
            
        if _emission_audit_rows:
            st.sidebar.divider()
            st.sidebar.markdown(f"##### 🧬 {_t('Dettaglio Audit Emissioni')}")
            # Ensure ep is updated in the audit rows for display
            for _r in _emission_audit_rows:
                if _r.get("Origine fattori") == EF_SOURCE_STD:
                    _r["ep usato"] = float(ep_total)
            st.sidebar.dataframe(pd.DataFrame(_emission_audit_rows), hide_index=True)

    # ============================================================
    # INCENTIVI & BUSINESS PLAN - Sidebar
    # ============================================================
    with st.sidebar.expander("💶 " + _t("Incentivi & Business Plan"), expanded=False):
        if IS_DM2022:
            bp_plant_type = st.sidebar.selectbox(_t("Tipo intervento"), list(BP_PLANT_TYPES.keys()), format_func=lambda x: BP_PLANT_TYPES[x]["label"], key="sb_bp_type")
            bp_dest_use   = st.sidebar.selectbox(_t("Destinazione"), list(BP_DEST_USE.keys()), key="sb_bp_dest")
            ghg_threshold = BP_DEST_USE[bp_dest_use]["ghg_thr"]
            FOSSIL_COMPARATOR = BP_DEST_USE[bp_dest_use]["cmp"]
        elif IS_DM2018:
            end_use = st.sidebar.selectbox(_t("Destinazione"), list(DM2018_END_USES.keys()), key="sb_dm18_dest")
            ghg_threshold = DM2018_END_USES[end_use]["sav"]
            FOSSIL_COMPARATOR = DM2018_END_USES[end_use]["cmp"]
            cic_price = st.sidebar.number_input("Valore CIC [€]", 0.0, 600.0, CIC_PRICE_DEFAULT, key="sb_cic_val")
            annex_threshold = ANNEX_IX_THRESHOLD
            advanced_mode = "Auto (calcolata da matrice annuale)"
        elif IS_FER2:
            fer2_tariffa_base = st.sidebar.number_input("Tariffa Base [€/MWh]", 0.0, 400.0, FER2_TARIFFA_BASE_DEFAULT, key="sb_fer2_tr")
            fer2_premio_matrice_eur = st.sidebar.number_input("Premio Matrice [€]", 0.0, 100.0, FER2_PREMIO_MATRICE_DEFAULT)
            fer2_premio_car_eur = st.sidebar.number_input("Premio CAR [€]", 0.0, 50.0, FER2_PREMIO_CAR_DEFAULT)
            ghg_threshold = 0.80
            FOSSIL_COMPARATOR = 183.0
            fer2_matrice_threshold = FER2_FEEDSTOCK_REQ_THRESHOLD
        else:
            ghg_threshold = 0.80
            FOSSIL_COMPARATOR = 80.0

        # --- Summary Metrics (Moved from Right) ---
        st.sidebar.divider()
        if IS_DM2022:
            _f = "fascia_a" if plant_net_smch <= BP_SOGLIA_FASCIA_SMCH else "fascia_b"
            _t_ref = BP_TARIFFE_REF.get(bp_plant_type, {}).get(_f, BP_TARIFFA_BASE_2026)
            st.sidebar.metric("Tariffa Asta", f"{fmt_it(_t_ref * (1 - BP_RIBASSO_DEFAULT_PCT/100.0), 1)} €/MWh")
        elif IS_DM2018:
            st.sidebar.metric("Prezzo CIC", f"{fmt_it(cic_price, 0)} €/CIC")
        elif IS_FER2:
            st.sidebar.metric("Tariffa FER2", f"{fmt_it(fer2_tariffa_base + fer2_premio_matrice_eur + fer2_premio_car_eur, 0)} €")

        target_saving = ghg_threshold + 0.01
...
# ============================================================
tab_solver, tab_daily, tab_export = st.tabs([
    "🎯 " + _t("Pianificazione Mensile"),
...
with tab_solver:
    ...
    # ------------------------- MODE SELECTOR -------------------------
    # MODE SELECTOR MOVED TO SIDEBAR
    
    
    # --- Solver configuration moved up ---
...
with tab_export:
    ...
    if _HAS_OUTPUT_MODEL:
        try:
            ...
            with st.sidebar.expander(_t(
                "📚 Origine dei dati e metodo di calcolo "
                "(rese, fattori emissivi, GHG, base normativa)"),
                expanded=False,
            ):
                st.sidebar.markdown(f"**{_t('Origine delle rese biomassa')}**")
                st.sidebar.text(_expl.get("yield_origin", ""))
                st.sidebar.markdown(f"**{_t('Origine dei fattori emissivi')}**")
                st.sidebar.text(_expl.get("emission_factor_origin", ""))
                st.sidebar.markdown(f"**{_t('Metodo di calcolo GHG (RED III)')}**")
                st.sidebar.text(_expl.get("ghg_method", ""))
                st.sidebar.markdown("**Base normativa applicata**")
                st.sidebar.text(_expl.get("regulatory_basis", ""))
